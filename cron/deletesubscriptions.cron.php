	...
	if(count($ArrFollower)){
		foreach($ArrFollower as $key => $Frs){
			...
			if($follower){
				...
				/* Comprueba si ya está vencida la suscripción del usuario */
				if ($ArrFollower[ $row['id'] ] < $ActualTime)
				{
					...
					/* Registra en la bd */
					$description = $connect->query("INSERT `notificaciones_suscripcionesvencidas` (usera,userb,see) VALUES ('$row[id]','$subscrip[id]','0')");

				}else{
					echo "Suscripcion Vence para ". date('d:m:y', $ArrFollower[$row['id']]) . PHP_EOL;
					continue;
				}
