...
$.fn.jqPuzzle = function(settings, texts) {
	...
	return this.filter('img').each(function(){
		...
		$srcImg.one('load', function() {
			...
			$srcImg.css({width: '100%', height: '100%', visibility: 'visible'});
			...
			var backgroundSize = ((width*3)+coveredWidth) +'px '+ ((height*3)+coveredHeight) + 'px';
			var backgroundFullSize = $srcImg.width() +'px '+ $srcImg.height() + 'px';
			...
			var $background = $('<div/>')
				.css({
					...
					backgroundSize: backgroundFullSize
			...
			var $credits = $('<a/>')
				...
				.attr('onclick', 'parent.loadgalery(<?php echo $_GET['randomImage']; ?>)')
				.css({
					'float': 'right',
