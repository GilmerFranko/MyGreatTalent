...
if (isset($_POST['precomprar'])){
 ...
 $(document).ready(function() {
  ...
  <center>
  <div class="badge badge-info" style="white-space: unset;"><h4>Confirmar la Compra del pack de '.$name['username'].'</h4></div>
  ...
  <div class="content-wrapper">
   ...
   <div id="content-container">
    <section class="content-header">
     <h1><i class="fas fa-layer-group"></i> Packs en venta</h1>
    </section>
        ...
        <table id="dt-basic" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
         ...
         <tbody>
          ...
          <center>
           ...
           <a href="mispackscomprados.php" class="item"> <button class="btn btn-success"><i class="fa fa-heart"></i> Mis Pack Comprados</button></a>
...
else
{
 ...
 $querycp = mysqli_query($connect, "SELECT *, pack.`id` as pack_id FROM `packsenventa` as pack INNER JOIN players AS `p` ON p.`id` = pack.`player_id` LEFT JOIN `friends` AS f ON (f.`player1` = pack.`player_id` && f.`player2` = '$rowu[id]') || (f.`player2` = pack.`player_id` && f.`player1` = '$rowu[id]') $WHERE $hidden_old ORDER BY pack.`id` DESC LIMIT {$calc_page}, {$num_results_on_page}");
}
}
/**
* SINO
   ...
   <td class="items box">
    <div class="">
     ...
     <div class="card-body comment-emoticons">
      <center><img src="<?php echo $sitio['site'].$Image;?>" width="100%"> </center><br> <?php
