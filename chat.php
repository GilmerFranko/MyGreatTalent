...
if (isset($_GET['deleteMsg']) and !empty($_GET['deleteMsg']))
{
	...
	$select = mysqli_query($connect, "SELECT id,author,foto,foto_unica, rutadefoto FROM `nuevochat_mensajes` WHERE id= '$idtrash'");
	...
	if ($select and $select->num_rows > 0)
	{
		...
		if ($row_delete['author'] == $rowu['id'] or $rowu['role'] == "Admin")
		{
			...
			{
				if ($row_delete['foto_unica'] == 1)
				{
					/* Comprueba si esta foto se sigue utilizando en otros mensajes, de ser asi, no la elimina */
					$foto_unica = $connect->query('SELECT id FROM `nuevochat_mensajes` WHERE `rutadefoto` = \'' . $connect->real_escape_string($row_delete['rutadefoto']) . '\'');
