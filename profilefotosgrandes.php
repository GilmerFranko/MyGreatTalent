...
<?php
		...
		if ($rowu['role'] == 'Admin'){
			if ($userbuscado["baneado"] == 0){
				echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&banear='. $userbuscado["baneado"] .'" class="btn btn-danger"><i class="fa fa-times"></i> Banear Usuario</a><br><br>';
			}else{
				echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&banear='. $userbuscado["baneado"] .'" class="btn btn-danger"><i class="fa fa-handshake"></i> Perdonar Usuario</a><br><br>';
			}
			//BOTON NO-PUBLIC
			$page = isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
			echo '<a href="'.$sitio['site'].'profile.php?page='.$page.'&profile_id=' . $userbuscado['id'] . '&action_nopublic='. $userbuscado["id"] .'" class="btn btn-primary"><i class="fa fa-action"></i> Cambiar a No-Public</a><br><br>';
			if ($userbuscado["permission_upload"] == 0){
				echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&permission_upload=1" class="btn btn-success"><i class="fa fa-check"></i> Permitir subir fotos</a><br><br>';
			}else{
				echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&permission_upload=0" class="btn btn-danger"><i class="fa fa-lock"></i> Bloquear subir fotos</a><br><br>';
			}
		}

		if (!isFollow($userbuscado["id"])){
			if($rowu['eCreditos']>=2000){
				echo '<a href="#" data-username="'.$userbuscado['username'].'" data-href="'.$sitio['site'].'profile.php?profile_id='.$userbuscado['id'].'&Subscribe" id="suscribe" class="btn btn-success">
					Suscribirme por <br/>2000 créditos especiales 7 días
				</a><br><br>';
			}else{
				echo '<a class="btn btn-success">Suscribirme por <br/> 2000 créditos especiales 7 días</a><br><br>';
			}
		}else{
			echo '<a class="btn btn-primary"><i class="fa fa-star"></i> Suscrito por 7 días</a><br><br>';
		}

		if($userbuscado['gender']=='mujer'){
			echo '<a class="btn btn-primary" onclick="openDonarCreditos()"><i class="fa fa-coins"></i> Donar Creditos</a><br><br>';
		}
		...
		if ($sonamigos == 'si'){
			echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&eliminar_id=' . $userbuscado['id'] . '" class="btn btn-danger"><i class="fa fa-user-times"></i> Eliminar Amistad</a>
		<br><br>';
			
		}elseif($sonamigos == 'no' && $hayunasolicitud == 'si'){
			
			echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&cancelar_id=' . $userbuscado['id'] . '" class="btn btn-danger"><i class="fa fa-user-times"></i> Cancelar Solicitud de amistad</a>
		<br><br>';
			...
			echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&agregar_id=' . $userbuscado['id'] . '" class="btn btn-primary"><i class="fa fa-user-plus"></i> Agregar Amistad</a>
			<br><br>';
		}else{
			
			echo '<button name="botondeshabilitado" id="botondeshabilitado" type="submit" class="btn btn-large btn-primary" disabled><i class="fa fa-user-plus"></i> Agregar Amistad</button><br><br>';
		...
		<button name="iniciarreporte" type="submit" class="btn btn-warning">
		...
		if($userbuscado['gender'] == 'hombre'){
			if ($countbloqueo < 1){
				echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&bloquear_id=' . $userbuscado['id'] . '" name="bloquear" type="submit" class="btn btn-danger"><i class="fa fa-times-circle"></i> Bloquear Perfil</a>';
			}else{
				echo '<a href="'.$sitio['site'].'profile.php?profile_id=' . $userbuscado['id'] . '&desbloquear_id=' . $userbuscado['id'] . '" name="bloquear" type="submit" class="btn btn-danger"><i class="fa fa-unlock"></i> Desbloquear Perfil</a>';
			}
		}
	
		if ($countbloqueo < 1){	
			echo '<br><br>
			<a href="'. $sitio['site'].$link .'" name="enviarunmensaje" type="button" class="btn btn-success"><i class="fas fa-envelope"></i> Iniciar chat con ' . $userbuscado['username'] . '</a>
			
			<hr>';
		}else{	
			echo '<br><br>
			<button name="botondeshabilitado" id="botondeshabilitado" type="submit" class="btn btn-large btn-success" disabled>Iniciar chat con ' . $userbuscado['username'] . '</button>
			<hr></center>';
		}
	}
	...
	<div class="card text-left" style="position: relative;">
		<div class="card-header bg-secondary mb-3">
			...
			<div style="position:absolute;top:0;right:0;">
				<a href="<?php Echo $rowcp['linkdedescarga'];?>" class="btn btn-primary"><i class="fa fa-link"></i></a>
			<?php }else{ ?>
			<!--BOTON LINK PARA REDIRIGIRSE AL POST-->
			<div style="position:absolute;top:0;right:0;">
				<a class="" href="<?php Echo $sitio['site'] .'foto.php?fotoID='. $rowcp['id'];?>">
	...
	?>
		...
		<div class="card-body comment-emoticons">
			...
			<center>
		        ...
		        <?php
		        //SI NO EXISTEN IMAGENES QUE MOSTRAR
		         if ($rowcp['imagen']!=null){
		        	Echo getSource($sitio['site'].$rowcp['imagen'], $sub, $sitio['site'].'foto.php?fotoID='.$rowcp['id']);
		       	//ENTONCES COLOCA EL BOTON VER CONTENIDO
		        }else{ ?>
					...
					style="background-image: url(<?php Echo $sitio['site'] . ($rowcp['thumb'] ? strdr($rowcp['thumb']):strdr($rowcp['imagen'])); ?>);">
					<!--LINK DE DESCARGA-->
					...
					<div class="noSubMessage">
						...
						<?php 
							...
							if (!isFollow($rowcpd["id"])){
								if($rowu['eCreditos']>=2000){
									echo '<a href="#" data-username="'.$rowcpd['username'].'" data-href="'.$sitio['site'].'profile.php?profile_id=' . $rowcpd['id'].'&Subscribe" id="suscribe" class="btn btn-primary">
										suscribirse a '.$rowcpd['username'].' por <br/>2000 créditos especiales 7 días</a><br><br>';
								}else{
									echo '<a class="btn btn-danger">Suscribirme por <br/>2000 créditos especiales 7 días</a><br><br>';
								}
							}else{
								echo '<a class="btn btn-primary"><i class="fa fa-star"></i> Suscrito por 7 días</a><br><br>';
							}
